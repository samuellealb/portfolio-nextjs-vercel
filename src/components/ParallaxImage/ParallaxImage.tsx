...
export const ParallaxImage = ({
  ...
  useEffect(() => {
    const translateImage = (id: string) => {
      ...
      const maxTranslate = (imageRect.height - wrapperRect.height) / 2;
      const imageTranslate = (scrollPosition / 100) * maxTranslate;

      if (
        wrapperRect.top < windowBottom + wrapperRect.height &&
        wrapperRect.bottom >= 0
      ) {
        image.style.transform = `translateY(${imageTranslate}px) scale(1.5)`;
      }
    };
